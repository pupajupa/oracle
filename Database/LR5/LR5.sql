...
CREATE OR REPLACE DIRECTORY MY_DIR AS 'D:\3 course\DB\OracleDB\Database\LR5\reports';
...
BEGIN
  ...
  BEGIN
    ...
    write_html('  <style>');
    write_html('    body {');
    write_html('      font-family: "Segoe UI", Tahoma, Geneva, Verdana, sans-serif;');
    write_html('      background-color: #eef1f5;');
    write_html('      color: #2c3e50;');
    write_html('      margin: 0;');
    write_html('      padding: 0;');
    write_html('    }');

    write_html('    .container {');
    write_html('      max-width: 960px;');
    write_html('      margin: 40px auto;');
    write_html('      padding: 0 20px;');
    write_html('    }');

    write_html('    .header {');
    write_html('      background-color: #1e3a8a;');
    write_html('      color: #ffffff;');
    write_html('      padding: 30px;');
    write_html('      border-radius: 8px 8px 0 0;');
    write_html('    }');

    write_html('    .report-title {');
    write_html('      font-size: 28px;');
    write_html('      font-weight: 600;');
    write_html('      margin-bottom: 10px;');
    write_html('    }');

    write_html('    .timestamp {');
    write_html('      font-size: 14px;');
    write_html('      opacity: 0.85;');
    write_html('    }');

    write_html('    .card {');
    write_html('      background-color: #ffffff;');
    write_html('      border-radius: 8px;');
    write_html('      box-shadow: 0 2px 8px rgba(0, 0, 0, 0.05);');
    write_html('      padding: 20px;');
    write_html('      margin-top: 30px;');
    write_html('    }');

    write_html('    .section-title {');
    write_html('      font-size: 20px;');
    write_html('      font-weight: 500;');
    write_html('      margin-bottom: 20px;');
    write_html('    }');

    write_html('    table {');
    write_html('      width: 100%;');
    write_html('      border-collapse: collapse;');
    write_html('    }');

    write_html('    th, td {');
    write_html('      padding: 12px 16px;');
    write_html('      text-align: left;');
    write_html('    }');

    write_html('    th {');
    write_html('      background-color: #f0f4f8;');
    write_html('      font-weight: 600;');
    write_html('      border-bottom: 2px solid #d1d5db;');
    write_html('    }');

    write_html('    tr:nth-child(even) {');
    write_html('      background-color: #f9fafb;');
    write_html('    }');

    write_html('    .badge {');
    write_html('      display: inline-block;');
    write_html('      padding: 4px 10px;');
    write_html('      border-radius: 16px;');
    write_html('      font-size: 13px;');
    write_html('      font-weight: 500;');
    write_html('      color: #fff;');
    write_html('    }');

    write_html('    .insert-badge { background-color: #10b981; }');  -- зелёный
    write_html('    .update-badge { background-color: #3b82f6; }');  -- синий
    write_html('    .delete-badge { background-color: #ef4444; }');  -- красный

    write_html('    .total-row {');
    write_html('      background-color: #e5e7eb;');
    write_html('      font-weight: bold;');
    write_html('    }');

    write_html('    .footer {');
    write_html('      text-align: center;');
    write_html('      padding: 20px;');
    write_html('      font-size: 13px;');
    write_html('      color: #6b7280;');
    write_html('      margin-top: 40px;');
    write_html('    }');
    write_html('  </style>');
    write_html('</head>');
    write_html('<body>');
