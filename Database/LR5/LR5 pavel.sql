...
BEGIN
  ...
  BEGIN
    ...
    write_html('      <h3 class="report-title">Отчет об изменениях базы данных</h3>');
    ...
    write_html('      <h3 class="section-title">Сводка изменений</h3>');
    write_html('      <table>');
    write_html('        <thead>');
    write_html('          <tr>');
    ...
    IF p_include_details THEN
      IF (v_cust_inserts + v_cust_updates + v_cust_deletes) > 0 THEN
         write_html('    <div class="card">');
        write_html('      <h3 class="section-title">Изменения клиентов</h3>');
        write_html('      <table>');
        write_html('        <thead>');
        write_html('          <tr>');
      ...
      IF (v_prod_inserts + v_prod_updates + v_prod_deletes) > 0 THEN
          write_html('    <div class="card">');
        write_html('      <h3 class="section-title">Изменения товаров</h3>');
        write_html('      <table>');
        write_html('        <thead>');
        write_html('          <tr>');
        write_html('            <th>Время</th>');
      ...
      IF (v_order_inserts + v_order_updates + v_order_deletes) > 0 THEN
         write_html('    <div class="card">');
        write_html('      <h3 class="section-title">Изменения заказов</h3>');
        write_html('      <table>');
        write_html('        <thead>');
        write_html('          <tr>');
        write_html('            <th>Время</th>');
