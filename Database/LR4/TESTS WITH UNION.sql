...
EXCEPTION
  WHEN OTHERS THEN
    RAISE_APPLICATION_ERROR(-20001, 'Error: ' || SQLERRM || '. SQL: ' || v_sql);
END;
/
...
BEGIN
  ...
  DBMS_OUTPUT.PUT_LINE('Results UNION:');
  LOOP
...
BEGIN
  v_cur := json_select_handler_union(v_json);
  DBMS_OUTPUT.PUT_LINE('UNION with different data types:');
  LOOP
      ...
      "queries": [
        {
          ...
          "where": {
            "conditions": [
              {"column": "instructor", "operator": "LIKE", "value": "%Teacher #1%"}
            ]
          }
        }
      ]
...
BEGIN
  v_cur := json_select_handler_union(v_json);
  DBMS_OUTPUT.PUT_LINE('UNION with WHERE conditions:');
  LOOP
    FETCH v_cur INTO v_col1, v_col2;
    EXIT WHEN v_cur%NOTFOUND;
    DBMS_OUTPUT.PUT_LINE(v_col1 || ' | ' || v_col2);
